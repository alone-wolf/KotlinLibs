...
sealed class CommandOperator(val symbol: String) {
    ...
    data class RedirectOutput(val append: Boolean = false) : CommandOperator(if (append) ">>" else ">")
    data class RedirectInput(val file: File) : CommandOperator("<")
    data class RedirectError(val append: Boolean = false) : CommandOperator(if (append) "2>>" else "2>")
}
