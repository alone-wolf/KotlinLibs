package top.writerpass.rekuester.tables.v2
...
private fun Table2(
    ...
    Column(
        ...
        if (headers.isNotEmpty()) {
            val rowState = state.rememberRowState(0)
            FullWidthRow(modifier = Modifier.height(rowState.height)) {
                VerticalDivider()
                headers.forEachIndexed { index, header ->
                    val columnState = state.rememberColumnState(index)
                    val dateValue = remember { header }
        ...
        dataSet.forEachIndexed { i, dataRow ->
            val i = if (headers.isNotEmpty()) i + 1 else i
